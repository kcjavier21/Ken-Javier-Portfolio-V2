...
import fieldworkStudio from "../../../../assets/images/jpg-png/FieldworkStudio.png"
...
const Testimonials = props => {
  ...
  const organizations = [
    ...
    { name: "Fieldwork Studio", image: fieldworkStudio, className: "fieldworkStudio" },
    {
    ...
    return (
      <>
        ...
        <i
          ...
          onClick={() => {
            slideNumber <= nodes.length && slideNumber >= 1
  ...
  const generateSlideBeadsForLargeDevices = index => {
    ...
    return counter <= half ? (
      <span
        key={index * 2}
        className={index * 2 === slideNumber ? `${styles.active}` : ""}
      />
    ) : (
      ""
    )
  ...
  return (
    <main className={styles.testimonials}>
      ...
      <div className={styles.testimonialsSlideShowContainer}>
        {/* ==== Control Buttons ===== */}
        {props.innerWidth > 834
          ? generateControlButtonsForLargeDevices()
          : generateControlButtonsForSmallDevices()}
        ...
        <div className={styles.slideBeads}>
          ...
          <div className={styles.slideBeads}>
            {nodes.map((item, index) =>
              props.innerWidth > 834
                ? generateSlideBeadsForLargeDevices(index)
